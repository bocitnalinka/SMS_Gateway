package com.example.smsgateway;//package com.example.smsreceiving;
...
//        String log_f_name="SMSGatewayLog_" + c_date.substring(0,4) + "_" + c_date.substring(5,7) + "_" + c_date.substring(8,10) + ".txt"; 		// File logfile = new File("ATMSMSLog.txt");
//
...
public class LogFile {
    ...
    public static void info(String text) {
        ...
        Calendar cal = Calendar.getInstance();
        String c_date = date_Format.format(cal.getTime());
        String log_f_name = "SMSLog_" + c_date.substring(0, 4) + "" + c_date.substring(5, 7) + "" + c_date.substring(8, 10) + ".txt";
        ...
        try {
            FileWriter file = new FileWriter(logfile, true); // Create Log file if not exist
            BufferedWriter log = new BufferedWriter(file);
            log.write("[" + currentDateTime + "] " + text);
            log.write("\r\n");
            log.close(); // Close the output stream
        } catch (Exception e) {
            System.err.println("Error in Log File: " + e.getMessage());
